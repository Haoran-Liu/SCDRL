...
for (i_percentage in percentage) {
    for (i_seed in seed) {
        file_path <- paste0("results/SCDRL/SCDRL_mouse_human_",
                        i_percentage, "_",
                        i_seed, '.npz')
        ...
        SCDRL_f1_system <- c(SCDRL_f1_system, SCDRL_performance[2, 1])
        SCDRL_f1_cell_type <- c(SCDRL_f1_cell_type, SCDRL_performance[2, 2])
        
        SCDRL_ARI_system <- c(SCDRL_ARI_system, SCDRL_performance[3, 1])
        SCDRL_ARI_cell_type <- c(SCDRL_ARI_cell_type, SCDRL_performance[3, 2])
    }
}
...
biolord_ARI_system <- c()
biolord_ARI_cell_type <- c()

for (i_percentage in percentage) {
    for (i_seed in seed) {
        file_path <- paste0("results/biolord/biolord_mouse_human_",
                        i_percentage, "_",
                        i_seed, '.npz')
        npzfile <- np$load(file_path)
        ...
        biolord_f1_system <- c(biolord_f1_system, biolord_performance[2, 1])
        biolord_f1_cell_type <- c(biolord_f1_cell_type, biolord_performance[2, 2])
        
        biolord_ARI_system <- c(biolord_ARI_system, biolord_performance[3, 1])
        biolord_ARI_cell_type <- c(biolord_ARI_cell_type, biolord_performance[3, 2])
    }
}
...
tmp_df_1 <- data.frame("percentage"=rep(percentage, each=10),
                        "seed"=seed,
                        "system" = biolord_f1_system,
                        "cell_type" = biolord_f1_cell_type
                )

tmp_df_2 <- data.frame("percentage"=rep(percentage, each=10),
                        "seed"=seed,
                        "system" = biolord_ARI_system,
...
for (i_percentage in percentage) {
    ...
    tmp_df <- df[df$attribute == "system", ]
    tmp_df <- tmp_df[tmp_df$percentage == i_percentage, ]
    
    file_name = paste0("plot/figures/mouse_human_f1_",
                "system", "_",
                i_percentage, ".pdf"
                )
