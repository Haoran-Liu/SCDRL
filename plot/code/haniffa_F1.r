...
for (i_percentage in percentage) {
    for (i_seed in seed) {
        file_path <- paste0("results/SCDRL/SCDRL_haniffa_",
                        i_percentage, "_",
                        i_seed, '.npz')
        ...
        SCDRL_f1_status <- c(SCDRL_f1_status, SCDRL_performance[2, 1])
        SCDRL_f1_cell_type <- c(SCDRL_f1_cell_type, SCDRL_performance[2, 2])
        
        SCDRL_ARI_status <- c(SCDRL_ARI_status, SCDRL_performance[3, 1])
...
biolord_ARI_status <- c()
biolord_ARI_cell_type <- c()

for (i_percentage in percentage) {
    for (i_seed in seed) {
        file_path <- paste0("results/biolord/biolord_haniffa_",
                        i_percentage, "_",
                        i_seed, '.npz')
        npzfile <- np$load(file_path)
        ...
        biolord_f1_status <- c(biolord_f1_status, biolord_performance[2, 1])
        biolord_f1_cell_type <- c(biolord_f1_cell_type, biolord_performance[2, 2])
        
        biolord_ARI_status <- c(biolord_ARI_status, biolord_performance[3, 1])
...
tmp_df_1 <- data.frame("percentage"=rep(percentage, each=10),
                        "seed"=seed,
                        "status" = biolord_f1_status,
                        "cell_type" = biolord_f1_cell_type
                )

tmp_df_2 <- data.frame("percentage"=rep(percentage, each=10),
                        "seed"=seed,
                        "status" = biolord_ARI_status,
...
for (i_percentage in percentage) {
    ...
    tmp_df <- df[df$attribute == "status", ]
    tmp_df <- tmp_df[tmp_df$percentage == i_percentage, ]
    
    file_name = paste0("plot/figures/haniffa_f1_",
                "status", "_",
                i_percentage, ".pdf"
                )
