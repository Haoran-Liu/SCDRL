...
npzfile <- np$load('results/SCDRL/SCDRL_haniffa_0.05_9.npz')
print(npzfile$files)
...
npzfile <- np$load('SCDRL_data/haniffa.npz')
print(npzfile$files)
...
mapping_status <- c("Covid", "Healthy")
names(mapping_status) <- c(1, 2)

pbmc <- CreateSeuratObject(counts = counts, project = "haniffa")
...
pbmc <- FindClusters(pbmc, resolution = 0.856) # 18 clusters
...
df$status <- factors[test_idx + 1, 1]
df$cell_type <- factors[test_idx + 1, 2]

df$status <- df$status + 1 # python is 0-indexed
df$cell_type <- df$cell_type + 1

df$status <- mapping_status[df$status]
df$cell_type <- mapping_cell_type[df$cell_type]

pdf("plot/figures/UMAP_haniffa_ground_truth_status.pdf", width = 12, height = 9)
ggplot(df, aes(umap_1, umap_2)) + geom_point(aes(colour = factor(status))) +
...
pdf("plot/figures/UMAP_haniffa_ground_truth_cell_type.pdf", width = 12, height = 9)
...
# SCDRL
df$status <- SCDRL_predictions[, 1] + 1 # python is 0-indexed
df$cell_type <- SCDRL_predictions[, 2] + 1

df$status <- mapping_status[df$status]
df$cell_type <- mapping_cell_type[df$cell_type]

pdf("plot/figures/UMAP_haniffa_SCDRL_status.pdf", width = 12, height = 9)
ggplot(df, aes(umap_1, umap_2)) + geom_point(aes(colour = factor(status))) +
    theme_classic() + theme(axis.text = element_text(size = 25),
    axis.title.x = element_text(size = 25), axis.title.y = element_text(size = 25),
    legend.title = element_text(size = 25), legend.text = element_text(size = 30)) +
...
pdf("plot/figures/UMAP_haniffa_SCDRL_cell_type.pdf", width = 12, height = 9)
ggplot(df, aes(umap_1, umap_2)) + geom_point(aes(colour = factor(cell_type))) +
    theme_classic() + theme(axis.text = element_text(size = 25),
    axis.title.x = element_text(size = 25), axis.title.y = element_text(size = 25),
    legend.title = element_text(size = 25), legend.text = element_text(size = 20)) +
...
# biolord
npzfile <- np$load('results/biolord/biolord_haniffa_0.05_9.npz')
print(npzfile$files)
biolord_predictions <- npzfile$f[["predictions"]]

df$status <- biolord_predictions[, 1] + 1 # python is 0-indexed
df$cell_type <- biolord_predictions[, 2] + 1

df$status <- mapping_status[df$status]
df$cell_type <- mapping_cell_type[df$cell_type]

pdf("plot/figures/UMAP_haniffa_biolord_status.pdf", width = 12, height = 9)
ggplot(df, aes(umap_1, umap_2)) + geom_point(aes(colour = factor(status))) +
    theme_classic() + theme(axis.text = element_text(size = 25),
    axis.title.x = element_text(size = 25), axis.title.y = element_text(size = 25),
...
pdf("plot/figures/UMAP_haniffa_biolord_cell_type.pdf", width = 12, height = 9)
ggplot(df, aes(umap_1, umap_2)) + geom_point(aes(colour = factor(cell_type))) +
    theme_classic() + theme(axis.text = element_text(size = 25),
    axis.title.x = element_text(size = 25), axis.title.y = element_text(size = 25),
    legend.title = element_text(size = 25), legend.text = element_text(size = 25)) +
...
# scVI
scvi_path <- "results/scVI/scVI_haniffa_seed9.csv"
if (!file.exists(scvi_path)) {
    scvi_path <- "results/scVI_haniffa.csv"
}
...
pdf("plot/figures/UMAP_haniffa_scVI_cell_type.pdf", width = 12, height = 9)
ggplot(df, aes(umap_1, umap_2)) + geom_point(aes(colour = factor(cell_type))) +
    theme_classic() + theme(axis.text = element_text(size = 25),
    axis.title.x = element_text(size = 25), axis.title.y = element_text(size = 25),
    legend.title = element_text(size = 25), legend.text = element_text(size = 25)) +
...
# Seurat
seurat_path <- "results/seurat/Seurat_haniffa_0.05_9.csv"
if (!file.exists(seurat_path)) {
    seurat_path <- "results/Seurat_haniffa.csv"
}
...
pdf("plot/figures/UMAP_haniffa_Seurat_cell_type.pdf", width = 12, height = 9)
ggplot(df, aes(umap_1, umap_2)) + geom_point(aes(colour = factor(cell_type))) +
    theme_classic() + theme(axis.text = element_text(size = 25),
    axis.title.x = element_text(size = 25), axis.title.y = element_text(size = 25),
    legend.title = element_text(size = 25), legend.text = element_text(size = 25)) +
...
for (i_seed in seed) {
    ...
    file_name = paste0("performance/haniffa_0.05_",
                i_seed, ".csv")
    if (!file.exists(file_name)) {
        file_name = paste0("performance/_combined/haniffa_0.05_",
                    i_seed, ".csv")
    }
...
file_name = "performance/haniffa_0.05_9.csv"
if (!file.exists(file_name)) {
    file_name = "performance/_combined/haniffa_0.05_9.csv"
}
df <- read.csv(file_name, row.names = 1)
df <- t(df)
df <- data.frame(df)
...
pdf("plot/figures/haniffa_ARI.pdf", width = 9, height = 9)
