...
def _load_base_data_and_normalize() -> tuple[np.ndarray, np.ndarray]:
    # https://docs.scvi-tools.org/en/stable/tutorials/notebooks/scrna/MrVI_tutorial.html
    npzfile = np.load(ROOT / "SCDRL_data" / "haniffa.npz")
    print(npzfile.files)
...
def _build_config(seed: int, n_samples: int, n_genes: int, n_factors: int) -> dict:
    ...
    config["factor_names"] = ["status", "cell_type"]
    config["factor_sizes"] = [2, 18]
    config["factor_dim"] = 1
...
def run_experiment(
    *,
    ...
    # Deterministic split/shuffle per run
    np.random.seed(seed)
    ...
    np.savetxt("results/SCDRL_haniffa_mig.csv", mig, delimiter=",")
    np.savetxt(
        os.path.join(results_dir, f"SCDRL_haniffa_mig_{percentage}_{seed}.csv"),
        mig,
        delimiter=",",
    )
    np.savetxt("results/SCDRL_haniffa_sap.csv", sap, delimiter=",")
    np.savetxt(
        os.path.join(results_dir, f"SCDRL_haniffa_sap_{percentage}_{seed}.csv"),
        sap,
        delimiter=",",
    )

    np.savetxt(
        "results/SCDRL_haniffa_dci_informativeness.csv",
        dci_informativeness,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir, f"SCDRL_haniffa_dci_informativeness_{percentage}_{seed}.csv"
        ),
        dci_informativeness,
        delimiter=",",
    )
    np.savetxt(
        "results/SCDRL_haniffa_dci_disentanglement.csv",
        dci_disentanglement,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir, f"SCDRL_haniffa_dci_disentanglement_{percentage}_{seed}.csv"
        ),
        dci_disentanglement,
        delimiter=",",
    )
    np.savetxt(
        "results/SCDRL_haniffa_dci_completeness.csv",
        dci_completeness,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir, f"SCDRL_haniffa_dci_completeness_{percentage}_{seed}.csv"
        ),
        dci_completeness,
        delimiter=",",
    )
    ...
    np.savetxt(
        "results/SCDRL_haniffa_hungarian_matched.csv",
        hungarian_matched,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir, f"SCDRL_haniffa_hungarian_matched_{percentage}_{seed}.csv"
        ),
        hungarian_matched,
        delimiter=",",
    )
    np.savetxt(
        "results/SCDRL_haniffa_hungarian_leakage.csv",
        hungarian_leakage,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir, f"SCDRL_haniffa_hungarian_leakage_{percentage}_{seed}.csv"
        ),
        hungarian_leakage,
        delimiter=",",
    )
    ...
    # Keep the original outputs (overwritten each run), and also write per-run files.
    np.savetxt("results/SCDRL_haniffa_cor.csv", spearman_correlations, delimiter=",")
    np.savetxt(
        os.path.join(results_dir, f"SCDRL_haniffa_cor_{percentage}_{seed}.csv"),
        spearman_correlations,
        delimiter=",",
    )

    spearman_correlations_abs = np.abs(spearman_correlations)
    cg_status = np.mean(spearman_correlations_abs[0][:2]) - np.mean(
        spearman_correlations_abs[0][2:]
    )
    cg_cell_type = np.mean(spearman_correlations_abs[1][2:]) - np.mean(
        spearman_correlations_abs[1][:2]
    )
    cg_mean = (cg_status + cg_cell_type) / 2
    cg = [cg_status, cg_cell_type, cg_mean]

    np.savetxt("results/SCDRL_haniffa_cg.csv", cg, delimiter=",")
    np.savetxt(
        os.path.join(results_dir, f"SCDRL_haniffa_cg_{percentage}_{seed}.csv"),
        cg,
        delimiter=",",
    )
    ...
    path = os.path.join(results_dir, f"SCDRL_haniffa_{percentage}_{seed}.npz")
    np.savez(
        path,
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    # Batch mode: if provided, these override --seed/--percentage.
