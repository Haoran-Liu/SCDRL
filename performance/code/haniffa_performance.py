...
def _load_scvi_npz(*, percentage: float, seed: int):
    path = os.path.join("results", "scVI", f"scVI_haniffa_{percentage}_{seed}.npz")
...
def _load_seurat_predictions(*, percentage: float, seed: int) -> pd.DataFrame:
    tagged = os.path.join(SEURAT_DIR, f"Seurat_haniffa_{percentage}_{seed}.csv")
    base_new = os.path.join(SEURAT_DIR, "Seurat_haniffa.csv")
    base_legacy = os.path.join("results", "Seurat_haniffa.csv")
...
def _load_seurat_pca(*, percentage: float, seed: int):
    tagged = os.path.join(SEURAT_DIR, f"Seurat_haniffa_pca_{percentage}_{seed}.npy")
    base_new = os.path.join(SEURAT_DIR, "Seurat_haniffa_pca.npy")
    base_legacy = os.path.join("results", "Seurat_haniffa_pca.npy")
    try:
        if os.path.exists(tagged):
...
npzfile = np.load(os.path.join(REPO_ROOT, "SCDRL_data", "haniffa.npz"))
print(npzfile.files)
factors_base = npzfile["factors"]

try:
    scvi_latent_all = np.load("results/scVI_haniffa_latent.npy")
except FileNotFoundError:
...
for PERCENTAGE in PERCENTAGES:
    for SEED in SEEDS:
        ...
        # SCDRL
        PATH = f"results/SCDRL/SCDRL_haniffa_{PERCENTAGE}_{SEED}.npz"
        ...
        SCDRL_ARI_condition = adjusted_rand_score(
            factors[test_idx, 0], SCDRL_predictions[:, 0]
        )
        ...
        # biolord
        PATH = f"results/biolord/biolord_haniffa_{PERCENTAGE}_{SEED}.npz"
        if os.path.exists(PATH):
            npz_bl = np.load(PATH)
            ...
            biolord_accuracy_condition = accuracy_score(
            ...
            biolord_ARI_condition = adjusted_rand_score(
                factors[test_idx, 0], biolord_predictions[:, 0]
            )
            ...
            biolord_accuracy_condition = np.nan
            biolord_accuracy_cell_type = np.nan
            biolord_f1_condition = np.nan
            biolord_f1_cell_type = np.nan
            biolord_ARI_condition = np.nan
        ...
        if scvi_npz is not None and "leiden_test" in scvi_npz.files:
            ...
            # Fallback: last-run scVI labels (legacy behavior).
            scVI_predictions_base = pd.read_csv("results/scVI_haniffa.csv")
        ...
        performance = {
            "SCDRL": {
                "accuracy_condition": SCDRL_accuracy_condition,
                "accuracy_cell_type": SCDRL_accuracy_cell_type,
                "f1_condition": SCDRL_f1_condition,
                "f1_cell_type": SCDRL_f1_cell_type,
                "ARI_condition": SCDRL_ARI_condition,
            ...
            "biolord": {
                "accuracy_condition": biolord_accuracy_condition,
                "accuracy_cell_type": biolord_accuracy_cell_type,
                "f1_condition": biolord_f1_condition,
                "f1_cell_type": biolord_f1_cell_type,
                "ARI_condition": biolord_ARI_condition,
        ...
        _write_method_csvs(
            dataset="haniffa", percentage=PERCENTAGE, seed=SEED, df=performance
        )
        ##########
