...
def _mig_from_mi_scores(mi: np.ndarray, block: slice) -> float:
    """Compute a simple block-vs-rest MI gap (original Biolord scripts style)."""
    mi = np.asarray(mi)
    in_block = mi[block]
...
def run_experiment(
    *, seed: int, percentage: float, counts_base: np.ndarray, factors_base: np.ndarray
    ...
    adata.obs.loc[label_masks, "status"] = "Unknown"
    ...
    biolord.Biolord.setup_anndata(
        adata,
        categorical_attributes_keys=["status", "cell_type"],
        categorical_attributes_missing={"status": "Unknown", "cell_type": "Unknown"},
    )
    ...
    np.savetxt("results/biolord_haniffa_cor.csv", spearman_correlations, delimiter=",")
    np.savetxt(
        f"results/biolord/biolord_haniffa_cor_{percentage}_{seed}.csv",
        spearman_correlations,
        delimiter=",",
    )
    ...
    np.savetxt("results/biolord_haniffa_mig.csv", mig, delimiter=",")
    np.savetxt(
        f"results/biolord/biolord_haniffa_mig_{percentage}_{seed}.csv",
        mig,
        delimiter=",",
    )
