...
def _make_test_idx(seed: int, num_cells: int, percentage: float) -> np.ndarray:
    ...
    random_idx = np.arange(num_cells)
    rng.shuffle(random_idx)
    # Important: match the split logic used elsewhere in this repo.
...
def run_for_seed(
    *,
    ...
    # Keep the call site type-safe for Pylance by branching.
    ...
    np.save(
        os.path.join(results_dir, f"scVI_simulation_latent_seed{seed}.npy"), latent_all
    )
    ...
    # Save full clustering labels for this seed (in permuted order).
    import pandas as pd

    pd.DataFrame({"leiden": leiden_all}).to_csv(
        os.path.join(results_dir, f"scVI_simulation_seed{seed}.csv"), index=False
    )
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    parser.add_argument("--n_latent", type=int, default=22)
    parser.add_argument("--n_top_genes", type=int, default=1000)
    parser.add_argument("--leiden_resolution", type=float, default=1.12)
    ...
    for seed in seeds:
        print(f"[scVI_simulation] training seed={seed}")
        ...
        for percentage in percentages:
            print(f"[scVI_simulation] eval seed={seed} percentage={percentage}")
            ...
            # Disentanglement metrics can be expensive; subsample deterministically.
            ...
            # Save per-(seed, percentage) artifacts (aligned to permuted data).
            np.save(
                os.path.join(
                    args.results_dir,
                    f"scVI_simulation_latent_{percentage}_{seed}.npy",
                ),
                latent_test,
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_simulation_cor_{percentage}_{seed}.csv",
                ),
                cor,
                delimiter=",",
            )
            ...
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_simulation_dci_informativeness_{percentage}_{seed}.csv",
                ),
                dci_informativeness,
                delimiter=",",
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_simulation_dci_disentanglement_{percentage}_{seed}.csv",
                ),
                dci_disentanglement,
                delimiter=",",
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_simulation_dci_completeness_{percentage}_{seed}.csv",
                ),
                dci_completeness,
                delimiter=",",
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_simulation_hungarian_matched_{percentage}_{seed}.csv",
                ),
                hungarian_matched,
                delimiter=",",
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_simulation_hungarian_leakage_{percentage}_{seed}.csv",
                ),
                hungarian_leakage,
                delimiter=",",
            )

            # Bundle everything needed for downstream metrics.
            np.savez_compressed(
                os.path.join(
                    args.results_dir,
                    f"scVI_simulation_{percentage}_{seed}.npz",
                ),
                seed=int(seed),
    ...
    # Backward-compatible outputs (latest run only).
    if last_seed is not None and last_percentage is not None:
        _ensure_dir("results")
        if last_latent_all is not None:
            np.save("results/scVI_simulation_latent.npy", last_latent_all)
        if last_leiden_all is not None:
            import pandas as pd

            pd.DataFrame({"leiden": last_leiden_all}).to_csv(
                "results/scVI_simulation.csv", index=False
            )
        if last_random_idx is not None:
            np.save("results/scVI_simulation_random_idx.npy", last_random_idx)
