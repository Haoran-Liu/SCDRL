...
.parse_args <- function(argv) {
    # Minimal parser for: --key value (or --flag)
    out <- list(
        seed = 9L,
        percentage = 0.05,
...
pbmc <- RunPCA(pbmc, features = VariableFeatures(object = pbmc), npcs = 22)

# cor
X <- Embeddings(pbmc, reduction = "pca")
dir.create(args$out_dir, showWarnings = FALSE, recursive = TRUE)

base_pca_path <- file.path(args$out_dir, "Seurat_simulation_pca.npy")
tagged_pca_path <- file.path(
    args$out_dir,
    paste0("Seurat_simulation_pca_", args$percentage, "_", args$seed, ".npy")
)
...
for (idx in seq_len(ncol(factors))) {
    spearman_cor[idx, ] <- apply(X, 2, function(col) cor(col, factors[, idx], method = "spearman"))
}
...
base_cor_path <- file.path(args$out_dir, "Seurat_simulation_cor.csv")
tagged_cor_path <- file.path(
    args$out_dir,
    paste0("Seurat_simulation_cor_", args$percentage, "_", args$seed, ".csv")
)
if (isTRUE(args$write_base)) {
...
####################
####################
# KMeans clustering baseline. Prefer scran+bluster if available; fall back to base R.
...
if (
    ...
    kmeans_clusters <- scran::clusterCells(
        sce,
        use.dimred = "PCA",
        BLUSPARAM = bluster::KmeansParam(centers = 16)
    )
} else {
    warning(
        "Bioconductor packages (SingleCellExperiment/scran/bluster) not available; using base::kmeans on PCA embeddings."
    )
    # Use the same PCA coordinates already computed by Seurat.
    X_km <- Embeddings(pbmc, reduction = "pca")
    kmeans_clusters <- as.integer(kmeans(X_km, centers = 16, nstart = 10)$cluster)
}

print(table(kmeans_clusters))

df <- data.frame(seurat_clusters, kmeans_clusters)
base_pred_path <- file.path(args$out_dir, "Seurat_simulation.csv")
tagged_pred_path <- file.path(
    args$out_dir,
    paste0("Seurat_simulation_", args$percentage, "_", args$seed, ".csv")
)
if (isTRUE(args$write_base)) {
