...
def run_for_seed(
    *,
    ...
    adata.obs["status"] = factors[:, 0].astype(str)
    ...
    scvi.model.SCVI.setup_anndata(adata, layer="counts", batch_key="status")
    ...
    np.save(
        os.path.join(results_dir, f"scVI_haniffa_latent_seed{seed}.npy"), latent_all
    )
    ...
    pd.DataFrame({"leiden": leiden_all}).to_csv(
        os.path.join(results_dir, f"scVI_haniffa_seed{seed}.csv"), index=False
    )
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    parser.add_argument("--leiden_resolution", type=float, default=0.46)
    ...
    for seed in seeds:
        print(f"[scVI_haniffa] training seed={seed}")
        ...
        for percentage in percentages:
            print(f"[scVI_haniffa] eval seed={seed} percentage={percentage}")
            ...
            np.save(
                os.path.join(
                    args.results_dir,
                    f"scVI_haniffa_latent_{percentage}_{seed}.npy",
                ),
                latent_test,
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_haniffa_cor_{percentage}_{seed}.csv",
                ),
                cor,
                delimiter=",",
            )
            ...
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_haniffa_dci_informativeness_{percentage}_{seed}.csv",
                ),
                dci_informativeness,
                delimiter=",",
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_haniffa_dci_disentanglement_{percentage}_{seed}.csv",
                ),
                dci_disentanglement,
                delimiter=",",
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_haniffa_dci_completeness_{percentage}_{seed}.csv",
                ),
                dci_completeness,
                delimiter=",",
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_haniffa_hungarian_matched_{percentage}_{seed}.csv",
                ),
                hungarian_matched,
                delimiter=",",
            )
            np.savetxt(
                os.path.join(
                    args.results_dir,
                    f"scVI_haniffa_hungarian_leakage_{percentage}_{seed}.csv",
                ),
                hungarian_leakage,
                delimiter=",",
            )
            ...
            np.savez_compressed(
                os.path.join(
                    args.results_dir,
                    f"scVI_haniffa_{percentage}_{seed}.npz",
                ),
                seed=int(seed),
    ...
    if last_seed is not None and last_percentage is not None:
        ...
        if last_latent_all is not None:
            np.save("results/scVI_haniffa_latent.npy", last_latent_all)
        if last_leiden_all is not None:
            import pandas as pd

            pd.DataFrame({"leiden": last_leiden_all}).to_csv(
                "results/scVI_haniffa.csv", index=False
            )
        if last_random_idx is not None:
            np.save("results/scVI_haniffa_random_idx.npy", last_random_idx)
