...
.parse_args <- function(argv) {
    out <- list(
        seed = 9L,
        percentage = 0.05,
        npz_path = "SCDRL_data/haniffa.npz",
...
base_pca_path <- file.path(args$out_dir, "Seurat_haniffa_pca.npy")
tagged_pca_path <- file.path(
    args$out_dir,
    paste0("Seurat_haniffa_pca_", args$percentage, "_", args$seed, ".npy")
)
...
base_cor_path <- file.path(args$out_dir, "Seurat_haniffa_cor.csv")
tagged_cor_path <- file.path(
    args$out_dir,
    paste0("Seurat_haniffa_cor_", args$percentage, "_", args$seed, ".csv")
)
if (isTRUE(args$write_base)) {
...
pbmc <- FindClusters(pbmc, resolution = 0.9) # 18 clusters
...
if (
    ...
    kmeans_clusters <- scran::clusterCells(
        sce,
        use.dimred = "PCA",
        BLUSPARAM = bluster::KmeansParam(centers = 18)
    )
} else {
    warning(
    ...
    kmeans_clusters <- as.integer(kmeans(X_km, centers = 18, nstart = 10)$cluster)
}

print(table(kmeans_clusters))

df <- data.frame(seurat_clusters, kmeans_clusters)
base_pred_path <- file.path(args$out_dir, "Seurat_haniffa.csv")
tagged_pred_path <- file.path(
    args$out_dir,
    paste0("Seurat_haniffa_", args$percentage, "_", args$seed, ".csv")
)
if (isTRUE(args$write_base)) {
