...
def _build_config(seed: int, n_samples: int, n_genes: int, n_factors: int) -> dict:
    ...
    config["factor_names"] = ["system", "cell_type"]
    config["factor_sizes"] = [2, 17]
    config["factor_dim"] = 1
...
def run_experiment(
    *,
    ...
    np.savetxt("results/SCDRL_mouse_human_mig.csv", mig, delimiter=",")
    np.savetxt(
        os.path.join(results_dir, f"SCDRL_mouse_human_mig_{percentage}_{seed}.csv"),
        mig,
        delimiter=",",
    )
    np.savetxt("results/SCDRL_mouse_human_sap.csv", sap, delimiter=",")
    np.savetxt(
        os.path.join(results_dir, f"SCDRL_mouse_human_sap_{percentage}_{seed}.csv"),
        sap,
        delimiter=",",
    )

    np.savetxt(
        "results/SCDRL_mouse_human_dci_informativeness.csv",
        dci_informativeness,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir,
            f"SCDRL_mouse_human_dci_informativeness_{percentage}_{seed}.csv",
        ),
        dci_informativeness,
        delimiter=",",
    )
    np.savetxt(
        "results/SCDRL_mouse_human_dci_disentanglement.csv",
        dci_disentanglement,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir,
            f"SCDRL_mouse_human_dci_disentanglement_{percentage}_{seed}.csv",
        ),
        dci_disentanglement,
        delimiter=",",
    )
    np.savetxt(
        "results/SCDRL_mouse_human_dci_completeness.csv",
        dci_completeness,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir, f"SCDRL_mouse_human_dci_completeness_{percentage}_{seed}.csv"
        ),
        dci_completeness,
        delimiter=",",
    )
    ...
    np.savetxt(
        "results/SCDRL_mouse_human_hungarian_matched.csv",
        hungarian_matched,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir, f"SCDRL_mouse_human_hungarian_matched_{percentage}_{seed}.csv"
        ),
        hungarian_matched,
        delimiter=",",
    )
    np.savetxt(
        "results/SCDRL_mouse_human_hungarian_leakage.csv",
        hungarian_leakage,
        delimiter=",",
    )
    np.savetxt(
        os.path.join(
            results_dir, f"SCDRL_mouse_human_hungarian_leakage_{percentage}_{seed}.csv"
        ),
        hungarian_leakage,
        delimiter=",",
    )
    ...
    np.savetxt(
        "results/SCDRL_mouse_human_cor.csv", spearman_correlations, delimiter=","
    )
    np.savetxt(
        os.path.join(results_dir, f"SCDRL_mouse_human_cor_{percentage}_{seed}.csv"),
        spearman_correlations,
        delimiter=",",
    )

    spearman_correlations_abs = np.abs(spearman_correlations)
    cg_system = np.mean(spearman_correlations_abs[0][:2]) - np.mean(
        spearman_correlations_abs[0][2:]
    )
    cg_cell_type = np.mean(spearman_correlations_abs[1][2:]) - np.mean(
        spearman_correlations_abs[1][:2]
    )
    cg_mean = (cg_system + cg_cell_type) / 2
    cg = [cg_system, cg_cell_type, cg_mean]

    np.savetxt("results/SCDRL_mouse_human_cg.csv", cg, delimiter=",")
    np.savetxt(
        os.path.join(results_dir, f"SCDRL_mouse_human_cg_{percentage}_{seed}.csv"),
        cg,
        delimiter=",",
    )
    ...
    path = os.path.join(results_dir, f"SCDRL_mouse_human_{percentage}_{seed}.npz")
    np.savez(
        path,
